...
while ((match = classAttrRegex.exec(cleaned)) !== null) {
  match[1].split(/\s+/).filter(Boolean).forEach((c) => templateClasses.add(c));
}
...
while ((match = safelistRegex.exec(inputCss)) !== null) {
  match[1].split(/\s+/).filter(Boolean).forEach((c) => safelisted.add(c));
}
...
const missing = [...templateClasses].filter(
  (c) => !safelisted.has(c) && !customClasses.has(c)
);
