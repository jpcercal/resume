...
exports.render = async (resume) => {
  ...
  // Filter to displayed skills; highlighted first, then sorted by level desc.
  const displayedSkills = (skills || [])
    .filter((s) => s.meta && s.meta.display)
    .sort((a, b) => {
      const aH = a.meta && a.meta.highlighted ? 1 : 0
      const bH = b.meta && b.meta.highlighted ? 1 : 0
